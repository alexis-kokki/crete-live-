...
function normalizeEvent(row) {
  ...
  return {
    ...
    venue: row.venue?.name || row.venue_name_raw || "Χώρος προς επιβεβαίωση",
    region: row.region,
    city: row.city || "",
    address: row.address || row.venue?.address || "",
    latitude: row.latitude ?? row.venue?.latitude ?? null,
    longitude: row.longitude ?? row.venue?.longitude ?? null,
    price: row.price_text || "Δεν αναφέρεται",
    phone: row.phone || row.venue?.phone || "",
...
async function loadEvents() {
  eventsContainer.innerHTML = `
    <div class="empty">
      ...
      <h3>Φόρτωση live...</h3>
      <p>Παίρνουμε τα δεδομένα από τη βάση.</p>
    </div>
  `;
  ...
  const select = [
    ...
    "poster_url",
    "last_verified_at",
    "venue:venues(name,address,latitude,longitude,phone)",
    "event_artists(artist:artists(name))",
    "event_genres(genre:genres(name))"
  ].join(",");
  ...
  try {
    ...
    eventsContainer.innerHTML = `
      <div class="empty">
        <div class="empty-icon">!</div>
        <h3>Δεν φορτώθηκαν τα live</h3>
        <p>Υπήρξε πρόβλημα σύνδεσης με τη βάση. Δοκίμασε ανανέωση.</p>
      </div>
    `;
  }
}
  ...
  eventsContainer.innerHTML = filtered.map(e => `
    <article class="event-card">
      <div class="event-image-wrap">
        ...
        <span class="badge">${e.primaryGenre}</span>
        <button class="fav" aria-label="Αγαπημένο" data-id="${e.id}">♡</button>
      </div>
      <div class="event-body">
        <div class="event-meta">
          <span>${formatDate(e.event_date)} · ${e.region}</span>
          <span>${e.time}</span>
        </div>
        <h3 class="event-title">${e.title}</h3>
        <p class="event-venue">
          ${e.artists.length ? e.artists.join(", ") + " · " : ""}${e.venue}
        </p>
...
function mapsUrl(e) {
  ...
  const q = encodeURIComponent(`${e.venue} ${e.address} ${e.city} Κρήτη`);
...
function openModal(id) {
  ...
  document.getElementById("modalBadge").textContent = e.primaryGenre;
  document.getElementById("modalTitle").textContent = e.title;
  document.getElementById("modalVenue").textContent =
    `${e.artists.length ? e.artists.join(", ") + " · " : ""}${e.venue}`;
  document.getElementById("modalDate").textContent = formatDate(e.event_date);
  ...
  document.getElementById("modalDescription").textContent =
    e.description || "Δεν υπάρχει ακόμη περιγραφή.";

  document.getElementById("mapsLink").href = mapsUrl(e);

  const phoneLink = document.getElementById("phoneLink");
  if (e.phone) {
    phoneLink.href = `tel:${e.phone}`;
    phoneLink.textContent = "Κράτηση";
    phoneLink.style.opacity = "1";
    phoneLink.style.pointerEvents = "auto";
  } else if (e.booking_url) {
    phoneLink.href = e.booking_url;
    ...
    phoneLink.textContent = "Χωρίς κράτηση";
    phoneLink.style.opacity = ".55";
...
document.getElementById("submitEventBtn").addEventListener("click", () => {
  alert("Η φόρμα υποβολής θα συνδεθεί στο επόμενο βήμα με τον πίνακα submissions.");
});
